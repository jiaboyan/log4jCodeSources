...
public class PropertyConfigurator implements Configurator {
    ...
    public void doConfigure(Properties properties, LoggerRepository hierarchy) {
        //将hierarchy对象赋值给repository属性：LoggerRepository
        repository = hierarchy;
        //查找 log4j.debug 对应的value:
        String value = properties.getProperty(LogLog.DEBUG_KEY);
        if (value == null) {
            //如果为null，获取 log4j.configDebug 对应的value:
        ...
        // 获取 log4j.reset 对应的value:
        String reset = properties.getProperty(RESET_KEY);
        if (reset != null && OptionConverter.toBoolean(reset, false)) {
            hierarchy.resetConfiguration();
        }

        // 获取 log4j.threshold 对应的value:
        ...
        //解析root根对象上的相关配置：遍历配置文件的各个元素，添加到 根ROOT中来：
        configureRootCategory(properties, hierarchy);
        //解析loggerFactory:获取log4j.loggerFactory 的value:并实例化；一般配置文件中不设置该key;
        configureLoggerFactory(properties);
        //解析除rootLogger之外的其他logger以及render信息
    ...
    // 解析loggerFactory（factory用于创建logger对象）:
    protected void configureLoggerFactory(Properties props) {
        // 获取log4j.loggerFactory 的value:
    ...
    void configureRootCategory(Properties props, LoggerRepository hierarchy) {
        //给 effectiveFrefix赋值为 log4j.rootLogger ：
        String effectiveFrefix = ROOT_LOGGER_PREFIX;
        //查找配置文件中的 log4j.rootLogger 属性的值：
        String value = OptionConverter.findAndSubst(ROOT_LOGGER_PREFIX, props);

        //如果 log4j.rootLogger 属性的值为空，那么就获取 log4j.rootCategory 属性的值：
        if (value == null) {
            value = OptionConverter.findAndSubst(ROOT_CATEGORY_PREFIX, props);
            // 把 log4j.rootCategory 赋值给effectiveFrefix对象：
            effectiveFrefix = ROOT_CATEGORY_PREFIX;
        }
        if (value == null) {
            LogLog.debug("Could not find root logger information. Is this OK?");
        } else {
            //如果value不为空，获取logger容器中的 根logger：
            Logger root = hierarchy.getRootLogger();
            synchronized (root) {
                //设置同步锁：具体解析rootLogger对象：
    ...
    //具体解析rootLogger对象： 配置文件对象，logger根对象，“log4j.rootLogger”，“root”，配置文件中解析出“log4j.rootLogger”的值；
    void parseCategory(Properties props, Logger logger, String optionKey, String loggerName, String value) {

        LogLog.debug("Parsing for [" + loggerName + "] with value=[" + value + "].");
        // StringTokenizer对象解析log4j.rootLogger的值，该值以","分割：log4j.rootLogger = INFO, FILE, CONSOLE
        StringTokenizer st = new StringTokenizer(value, ",");
        //如果  INFO, FILE, CONSOLE 不以"," ，或者value为空:
        if (!(value.startsWith(",") || value.equals(""))) {
            if (!st.hasMoreTokens()) {
                return;
            }
            //得到value的第一个值：INFO，为rootLogger的日志级别；
            String levelStr = st.nextToken();
            LogLog.debug("Level token is [" + levelStr + "].");
            // 字符串 “inherited”是否等于字符串 "INFO"，或者是否为null:
            if (INHERITED.equalsIgnoreCase(levelStr) || NULL.equalsIgnoreCase(levelStr)) {
                ...
                //设置日志的级别：看OptionConverter.toLevel()方法如何实现：
                logger.setLevel(OptionConverter.toLevel(levelStr, (Level) Level.DEBUG));
            }
            LogLog.debug("Category " + loggerName + " set to " + logger.getLevel());
        }

        //在开始配置根Logger的Appender前,先删除并关闭Logger中所有的Appender:
        logger.removeAllAppenders();
        //Appender负责定义日志输出的目的地:
        Appender appender;
        String appenderName;
        //遍历st对象，看INFO（INFO, FILE, CONSOLE）后面的值：
        while (st.hasMoreTokens()) {
            // 将 "FILE"赋值给appenderName对象：
            appenderName = st.nextToken().trim();
            //如果appenderName为null,或者为","，则下一次循环：
            if (appenderName == null || appenderName.equals(","))
                continue;
            LogLog.debug("Parsing appender named \"" + appenderName + "\".");
            //这一步解析 Appender对象：FILE对应的配置属性；
            appender = parseAppender(props, appenderName);
            //不为空的话，添加到rootLogger对象中去；
            if (appender != null) {
    ...
    Appender parseAppender(Properties props, String appenderName) {
        //从PropertyConfigurator的Hashtable中获取appenderName ：
        ...
        //第一次配置，读取log4j.appender.[AppenderName]属性，生成相应的Appender实例。
        // log4j.appender.FILE=org.apache.log4j.FileAppender
        String prefix = APPENDER_PREFIX + appenderName;
        //log4j.appender. + appenderName + .layout
        String layoutPrefix = prefix + ".layout";

        //实例化appender对象：org.apache.log4j.FileAppender
        ...
        //设这Appender的名称为[AppenderName]。
        appender.setName(appenderName);
        //判断声称的Appender是否是OptionHandler的实例:
        if (appender instanceof OptionHandler) {
            //判断Appender是否需要Layout:FileAppender是WriterAppender的子类：，默认返回true;
            if (appender.requiresLayout()) {
                //通过  log4j.appender.FILE.layout=org.apache.log4j.PatternLayout  实例化:
                //其中pattern的值默认为 %m%n
                Layout layout = (Layout) OptionConverter.instantiateByKey(props, layoutPrefix, Layout.class, null);
                if (layout != null) {
                    appender.setLayout(layout);
                    LogLog.debug("Parsing layout options for \"" + appenderName + "\".");
                    //修改PatternLayout对象的pattern属性：log4j.appender.FILE.layout.conversionPattern
                    // 通过反射实现的，PropertyDescriptor属性描述器；
            ...
            //设置log4j.appender.FILE的其他属性，同上面一样，也是经过反射，PropertyDescriptor属性描述器来实现的；
            //给FileAppender的属性进行赋值：
            PropertySetter.setProperties(appender, props, prefix + ".");
            LogLog.debug("Parsed \"" + appenderName + "\" options.");
        }
        parseAppenderFilters(props, appenderName, appender);
        //向PropertyConfigurator中的 appender缓存中注册 此appender:
