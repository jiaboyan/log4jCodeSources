...
public class Category implements AppenderAttachable {
    ...
    // 这些方法由AppenderAttachableImpl类实现--c.aai
    ...
    public void callAppenders(LoggingEvent event) {
        ...
        for (Category c = this; c != null; c = c.parent) {
            synchronized (c) {
                ...
                //若该logger的additive属性为true，则调用父类的appender输出日志（additive属性默认为true）
                //下代码可以看出,如果additive属性为true,则会逐层调用父日志的appender，直到root或者遇到additive属性为false的祖先；
                if (!c.additive) {
                    break;
                }
            }
    ...
    /**
     * debug日志打印方法：Logger是Category的子类；
     * 如果调用debug方法打印日志，那么root根日志对象设置的级别必须为debug，否则不会打印日志；
     */
    public void debug(Object message) {
        // repository是在log4j初始化后获取Logger对象时候赋值的，
        // 就是LogManager静态代码块中的创建的 Hierarchy对象（日志容器）：

        //判断Hierarchy对象中设置的日志级别(为ALL)：是否大于Debug级别；如果大于则直接返回；
        if (repository.isDisabled(Level.DEBUG_INT)){
            return;
        }
        //debug方法判断  Debug级别 与 根root级别：  如果根root设置级别大于Debug，则结束，不打印日志；
    ...
    protected void forcedLog(String fqcn, Priority level, Object message, Throwable t) {
        //创建LoggingEvent实例:它是对一次日志记录过程中所能取到信息的抽象；
    ...
    /**
     * 获取有效的日志级别：从此日志对象开始遍历，逐渐父日志：
     * Logger类包含Level信息 ，如果当前Logger未设置Level值，它也可以中父节点中继承下来
     */
    public Level getEffectiveLevel() {
        //第一次遍历，此日志级别为null,获取父日志对象，一般为根Root日志对象：
        for (Category c = this; c != null; c = c.parent) {
            if (c.level != null) {
                //返回根日志对象的级别：
                return c.level;
    ...
    /**
     * info方法，打印日志：
     * 如果调用info方法打印日志，那么root根日志对象设置的级别必须为debug / info，否则不会打印日志；
     */
    public void info(Object message) {
        // repository是在log4j初始化后获取Logger对象时候赋值的，
        // 就是LogManager静态代码块中的创建的 Hierarchy对象（日志容器）：

        //判断Hierarchy对象中设置的日志级别(为ALL)：是否大于info级别；如果大于则直接返回；
        if (repository.isDisabled(Level.INFO_INT)) {
            return;
        }
        //info方法判断  Info级别 与 根root级别：  如果根root设置级别大于info，则结束，不打印日志；
        if (Level.INFO.isGreaterOrEqual(this.getEffectiveLevel())) {
            // 创建一个日志事件：开始日志打印的相关操作：
            // 传递参数为 ： Category.class.getName()，INFO日志级别，日志打印内容，null
